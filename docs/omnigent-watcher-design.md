...
Implement the watcher entirely in `~/dotfiles`. Do not modify
...
`services/omnigent-watcher` installs a stdio MCP server exposing two
surfaces over one engine:

- `diff_subscribe(session_id, diffs, events?)`
- `diff_unsubscribe(session_id, diff?)`
- `diff_status(session_id)`
- `subscribe(session_id, subject, command, extract?, interval_seconds?, timeout_seconds?)`
- `unsubscribe(session_id, subject?)`
- `status(session_id)`
...
`agent_config/plugins/custom-mcps/mcps/watch.json` -> `~/.claude.json`,
...
2. Bind synchronously through the shared `Watcher.subscribe`, which reads
...
Source scoping on those rows is the only thing keeping `diff_unsubscribe`
...
format 2 includes `watcher.sqlite3`, validates its checksum/schema summary,
...
```text
[Watcher dwb_...] D12345 has 2 unresolved review comments and 1
...
2. Check recent session items for `[Watcher <batch-id>]`.
...
`services/omnigent-watcher/config.toml` is source-controlled with:
