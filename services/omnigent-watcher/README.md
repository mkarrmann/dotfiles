...
- The hub-only service re-binds `watch_requests` rows that have no live
  ...
  in `~/.omnigent/watcher.sqlite3`. Set
  `OMNIGENT_WATCHER_DATABASE` to point both processes at another file.
...
`systemctl --user restart omnigent-watcher` — and only the sidecar may
