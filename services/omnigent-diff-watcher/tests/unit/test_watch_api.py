...
def _diff_watch(repository: WatcherRepository) -> str:
    diff = fixture("active")
    repository.request_watch(
...
def test_cancelling_generic_watches_never_reaches_a_diff(tmp_path: Path) -> None:
    ...
    subject = _diff_watch(repository)
...
def test_cancelling_diff_watches_never_reaches_a_generic_one(tmp_path: Path) -> None:
