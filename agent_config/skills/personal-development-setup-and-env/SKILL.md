...
- **AeroSpace auto-collapses single-child containers (when normalization is enabled).** With `enable-normalization-flatten-containers = false`, single-child containers persist. `arrange-workspaces` exploits this to create single-window accordion containers via join-then-extract (join with sidebar, move sidebar back out).
