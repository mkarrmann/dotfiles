...
- **`aerospace focus left/right` defaults to `--boundaries-action wrap-around-the-workspace`.** Bare `focus left` from the leftmost window wraps to the rightmost — focus *always* changes. Any "walk until focus stops moving" loop (e.g. `discover_spatial_order` in `arrange-workspaces`) infinite-loops without `--boundaries-action stop`. Confirmed on AeroSpace 0.20.3-Beta; the `alt-h/j/k/l` keybindings setting wrap-around explicitly are redundant (matches the default), not the cause.
