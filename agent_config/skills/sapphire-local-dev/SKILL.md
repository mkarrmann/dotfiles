...
- `/tmp/cosco_service_helper.log` is a **fixed path** both drivers redirect log4j to → it gets interleaved across concurrent runs. Only matters when debugging a _failed_ run; the per-run stdout log (where results land) is separate and safe.
