...
description: >-
  ...
  server. Covers the work-hub vs desktop-local topology, the localhost proxy gotcha, the
...
## Topology — a server on 127.0.0.1:6767, whichever machine you are on

The server is always dialed at **`$OMNIGENT_URL`** (default
`http://127.0.0.1:6767`; prefer it over hardcoding the port), but what answers
there depends on the machine:

- **Work: one always-on hub.** `~/dotfiles/systemd/omnigent-server.service`
  runs `omnigent server` on port 6767 against `~/.omnigent/chat.db`, on the
  hub devserver only — `OMNIGENT_PRIMARY_FQDN` in `~/dotfiles/omnigent_config/topology.env`
  (currently `devvm20365.cco0.facebook.com`; standby `devvm36111.ftw0`). Every
  other work consumer — the Mac, other devservers, Orchest, CodeCompanion —
  reaches it through an SSH/ET forward, so the REST API works the same
  everywhere the forward is up. The live DB exists only on the hub.
- **Linux desktop: its own local server.** `systemd/desktop/omnigent-host.service`
  runs `omnigent host --server ""`, which starts a local server on the same port
  with its own `~/.omnigent/chat.db`. Nothing forwards to the work hub; sessions
  there are not visible here and vice versa.

### ⚠️ Localhost proxy gotcha (read this first on a work host)

On Meta hosts a corp HTTP proxy is set in Matt's shells, and it **intercepts
loopback and returns `403 "127.0.0.1 is private"`**. Every `curl` to the server
MUST bypass it, or you'll misread a healthy server as down. The desktop sets no
proxy, where the flag is harmless:
...
ls -lh ~/.omnigent/chat.db 2>/dev/null && echo "server runs here"  # DB local?
```
...
| **C. On-disk SQLite**             | Yes              | No — **server host only** | REST unreachable AND you're on the machine running the server. |
...
## Path C — On-disk SQLite (fallback on the machine running the server)

Only when REST is down **and** you're on the machine running the server. The DB is
...
2. **Turn-level signal:** scan items for `type == "error"`. The most common
   ...
   > `RuntimeError: turn exceeded the <N>s harness idle watchdog (run_turn emitted no events for <N>s; likely a wedged LLM or tool call)`

   `N` is `HARNESS_TURN_TIMEOUT_S`: 1800 on the work host unit
   (`systemd/omnigent-host.service`), the 3600 default on the desktop. It fires
   when the outer harness receives **no progress events** for that long; it
   does not prove the model or tool is wedged. Check
...
- **`chat.db` lives only where the server runs** — the work hub, or the
  desktop's own local server. Elsewhere REST/CLI are your only options; a
