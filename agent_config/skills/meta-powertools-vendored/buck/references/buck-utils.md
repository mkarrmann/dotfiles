# Buck Utility Commands

Buck2 offers logging and debugging commands to help understand what is going on behind the scenes during a build.

## Logging

### Outputs the log in JSON format from selected invocation, the output could be very large

```bash
buck log show
buck log show --trace-id <uuid>
```

### Replay a previous build, show all the actions and the outputs. Use `--speed` to adjust playback speed.

```bash
buck log replay --trace-id <uuid> --speed 10
```

### Show the failed actions from a build

```bash
buck log what-failed
buck log what-failed --trace-id <uuid>
```

### Show what actions ran (very useful!), can have options for filtering or formatting.

```bash
buck log what-ran
buck log what-ran --trace-id <uuid>
buck log what-ran --trace-id <uuid> --format json
buck log what-ran --skip-cache-hits
buck log what-ran --trace-id <uuid> --skip-remote-executions
```

### Show what materialized, the output lists the path, the materialization method, the file count, and the total size (after decompression).

```bash
buck log what-materialized
buck log what-materialized --trace-id <uuid>
```
...
```bash
buck build --build-report report.json cell//path/to:target
```

### Streaming build report

```bash
buck build --streaming-build-report report.jsonl cell//path/to:target
```

## Debugging

Buck does provide a set of debug commands but it is mainly used to debug buck
itself. They are generally not for debugging user's build. Use `buck debug -h`
for the options.
