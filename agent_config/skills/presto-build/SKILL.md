...
- **Never run two `presto-build`/`presto-deploy` at once in the same checkout.** If you genuinely need parallelism, use a _different_ checkout (`~/checkout2`, `~/checkout3`) — each has isolated build state.
