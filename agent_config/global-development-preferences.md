...
- Before waiting for a build, job, deployment, or other changing condition, use the `waiting-without-polling` skill. Bias toward completion notifications, event subscriptions, or quiet background observers instead of repeatedly asking the model to check status. Prefer harness-native equivalents when their event coverage, lifetime, and latency fit the task; Omnigent's custom watcher is useful for persistent waits and specialized Meta CI/review feedback, not mandatory for every wait. A scheduled model check is still model polling. After arranging notification, continue other work or finish the turn; cancel observers when their task is done. Phabricator-specific follow-up remains covered by the Meta workspace instructions.
...
- DO prioritize solving and verifying problems in the minimal amount of wall time (without sacrificing quality and thoroughness). In particular, be prepared to escape early and search for alternatives if a step is taking longer than expected, and always look for ways to parallelize your work. Parallelization will include aggressive use of subagents, running different commands in parallel, and ensure individual tool calls (e.g. running unit tests) are configured to run in a parallel manner which minimizes wall time. While you should exercise best judgement, you should not be afraid to spend a large amount of compute/tokens in order to effectively, thoroughly, and rapidly solve problems–sub-agents allow you to do this, without causing context rot by filling up your own context window.
...
- DO: For one-off questions — clarifying a detail, looking something up, elaborating on one point — fork a subagent instead of working it inline: spin it up in its own context, branched from the current conversation state, and have it research and report back concisely, with the usual evidence (exact code links and snippets). The goal is more iterations — sub-threads of conversation that establish mutual understanding — without spending main-thread context on the dig.
